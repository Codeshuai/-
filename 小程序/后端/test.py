...
openai.api_key = "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"
...
def code_analysis(code, choice):
    if choice == '1':
        ...
        return openai_completion(code + prompt_Comments)
    elif choice == '2':
        # print(openai_completion(code + prompt_Function))
        return openai_completion(code + prompt_Function)
    elif choice == '3':
        # print(openai_completion(code + prompt_Idea))
        return openai_completion(code + prompt_Idea)
    elif choice == '4':
        # print(openai_completion(code + prompt_Knowledge))
        return openai_completion(code + prompt_Knowledge)
    elif choice == '5':
        # print(openai_completion(code + prompt_Simply))
        return openai_completion(code + prompt_Simply)
    elif choice == '6':
        # print(openai_completion(code + prompt_Rewrite))
        return openai_completion(code + prompt_Rewrite)
...
def Answer(query):
    ...
    如果query的id存在zai storage.json中进行查询判断
    '''
    data = read_json()
    has_id = False
    for record in data:
    ...
    if len(query['inputMsg']) > 5:
        for record in data:
            if record['id'] == query['id']:
                ...
                break
        # print('五个任务：\n1\n2\n3\n4\n5\n0代表退出')
        return jsonify({'Answer': 'Menu'})  # 告诉前端,可以输出Menu给用户选择
    else:  # query是选项
        # 该用户没输入代码
        # 先把数据读出到data，再赋值
        data = read_json()
        for record in data:
            if record['id'] == query['id'] and record['code'] == '':
                # print("请先输入代码")
                return jsonify({'Answer': 'Nocode'})  # 告诉前端,让对应用户先输入代码Nocode
        else:
            if query['inputMsg'] == '0':
                ...
                for record in data:
                    if record['id'] == query['id']:
                        ...
                        write_json(data)
                        # print("退出成功")
                        return jsonify({'Answer': 'Exit'})  # 告诉前端,让对应用户退出
                ...
                for record in data:
                    if record['id'] == query['id']:
                        ...
                        return jsonify({'Answer': analysis})  # 告诉前端,让对应用户选择
            else:
                # print('选项错误')
                return jsonify({'Answer': 'Overflow'})  # 告诉前端,用户选择溢出
